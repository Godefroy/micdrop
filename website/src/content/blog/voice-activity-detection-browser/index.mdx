...
That combination is why it became the default choice for voice agents. It separates a human voice from a television, a fan or a passing truck, which no volume threshold can do.
...
Micdrop turns Silero on with one option:
...
Four numbers control the behaviour, and Micdrop exposes them as constructor options:
...
The thresholds are deliberately low here. The upstream library defaults to 0.5, and Micdrop uses 0.18 because soft-spoken users get cut out at the higher value. A lower threshold buys more misfires, which the two-stage confirmation absorbs at no cost.
...
Detection always lags the sound that triggered it, so Micdrop delays the recorded stream. A volume VAD checking every 100 ms notices the word 100 ms after it started, and by then the attack of the word is gone. Micdrop routes the recorded audio through a Web Audio `DelayNode` set to the detection interval, so what gets captured starts slightly before the moment of detection. Words keep their first consonant.
...
In Micdrop the browser handles it without a server round trip. The moment speech is confirmed, playback stops locally, the processing state is cleared, and a message goes out telling the server to abandon the answer it was streaming. The cut lands immediately because it happens on the machine holding the microphone.
...
<FaqList>
  ...
  <FaqItem question="What is Silero VAD and what are its applications?">
    Silero VAD is a neural network of about 309K parameters that reads a 512-sample frame, 32 milliseconds at 16 kHz, and returns the probability that the frame contains speech. Its LSTM state carries across frames, so it scores each frame using the ones before it. A volume threshold measures loudness and nothing else. Silero separates a human voice from a television, a fan or a passing truck. Voice agents adopted it as their default detector, and Micdrop runs it in `@micdrop/client`, see [client-side VAD](/docs/client/vad).
  </FaqItem>
  <FaqItem question="Is Silero VAD open source?">
    Yes. Silero VAD ships under the MIT license, and Micdrop's browser build comes from `@ricky0123/vad-web`, which wraps the Silero ONNX model and runs it with ONNX Runtime Web.
  </FaqItem>
...
On a phone, `@micdrop/react-native` runs the volume detector by default, with Silero available as an extra native module. You also choose there where transcription happens, on the device or on your server. Read [speech to text in React Native](/blog/speech-to-text-react-native) to compare the two.
