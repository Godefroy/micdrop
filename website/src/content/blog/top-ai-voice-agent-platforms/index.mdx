...
[Micdrop](https://micdrop.dev) is a set of MIT-licensed TypeScript packages that run a voice agent inside a web application and the Node server behind it. The browser package handles the microphone, voice activity detection and playback. The server package runs speech-to-text, the model and text-to-speech over a WebSocket. A React Native package brings the same client to iOS and Android. The code runs on your own servers with your own provider keys, so you pay for those servers and for what OpenAI, Mistral, ElevenLabs, Cartesia, Gladia or Gradium bill you. Kokoro, Piper, Pocket TTS, Qwen TTS and Whisper run on your own hardware instead, with no provider bill.
