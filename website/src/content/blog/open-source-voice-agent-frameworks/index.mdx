...
Vapi, Retell AI, Bland and Synthflow are closed source platforms you rent rather than run. Three ship what the documentation calls a server SDK, a typed REST client rather than an orchestrator, while Synthflow ships only a REST API you call yourself. The voice loop stays on their infrastructure. Telnyx and the managed clouds of LiveKit and Pipecat also rent out that loop. Twilio ConversationRelay rents out speech-to-text and text-to-speech and leaves the model to your own server. Hosted voice agent platforms differ widely on [price per minute and EU hosting](/blog/top-ai-voice-agent-platforms).

Most of the seven criteria cannot be applied to a platform with no repository to read, so Vapi, Retell AI, Bland and Synthflow are ordered on the one axis that stays measurable: how much of the stack you keep control of, from Vapi where you can supply your own model keys down to Synthflow where there is no self-hosting and no SDK at all.
...
<FaqList>
  ...
  <FaqItem question="Can I build a voice AI agent entirely in TypeScript?">
    Yes, though only three projects qualify. LiveKit Agents ships a Node distribution with roughly half the plugin catalogue of its Python side, the OpenAI Agents SDK runs a server-side voice loop over WebSocket if you use OpenAI's realtime model, and Micdrop is TypeScript on both the browser and the server, with swappable providers or a realtime model from OpenAI or Google, though it is much the smallest and youngest of the three. Everything else labelled a Node or TypeScript SDK by a voice vendor is a REST client that calls a service running the conversation elsewhere.
  </FaqItem>
  ...
  <FaqItem question="Is Pipecat free?">
    Yes. Pipecat is BSD 2-Clause, and its smart-turn detection model carries the same licence, so you can run that model outside Pipecat too. Run the framework on your own servers and Pipecat adds nothing to the bill. A call costs what your speech-to-text, model and text-to-speech providers charge at their list price. Daily, the company behind Pipecat, also sells Pipecat Cloud, a managed version generally available since January 2026 at $0.01 to $0.03 per agent minute, with phone calls at $0.018. Those rates pay for Daily's hosting, while the framework you run yourself stays free.
  </FaqItem>
