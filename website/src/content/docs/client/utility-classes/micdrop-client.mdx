...
```typescript
import { MicdropClient } from '@micdrop/web'
...
- `disableInterruption`: If true, the microphone stops listening while the assistant speaks, so the user cannot interrupt it (default: false). It is forced on when the browser reports that echo cancellation is off, and `isInterruptionDisabled` tells whether it applies
...
- `Message`: Emitted for each message added to the conversation, provides a `MicdropConversationItem` object
- `PartialAssistantMessage`: Emitted while the answer is being written, provides the text so far. Sent by servers running with [`partialMessages`](../../server/partial-messages)
- `ToolCall`: Emitted when the agent calls a tool the client should know about, provides a `MicdropToolCall` object
- `Classification`: Emitted when the [classifier](../../server/classifier) of the server has a result for the user turn, provides a `MicdropClassification` object. Sent by servers running with `classifierOptions: { sendToClient: true }`
...
interface MicdropState {
  ...
  // The answer being written, when the server streams it. Empty otherwise
...
See more complete example in the [DevicesSettings](https://github.com/Godefroy/micdrop/blob/main/examples/advanced/client/src/components/DevicesSettings.tsx) component of the advanced example.
