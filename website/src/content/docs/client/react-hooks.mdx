---
title: 'React Hooks'
description: 'Read the state, errors and audio levels of a Micdrop call in your React components with six hooks, then copy a complete call component that uses them.'
order: 4
---
...
The same package covers React Native: the state and the hooks come from `@micdrop/client`, only `Micdrop` is then imported from `@micdrop/react-native`.
...
Receive what the [classifier](../server/classifier) of the server made of the
user turn, once the server is configured with
...
function IntentBadge() {
  ...
  const handleClassification = useCallback(({ result }) => {
    setIntent(result.answers.intent.choice)
  }, [])

  useMicdropClassification(handleClassification)

  return <span className="intent">{intent}</span>
}
```

The callback receives a `MicdropClassification`, with the `input` classified
(the turn and its history), the `result` in the shape the classifier returns,
and the `duration` in ms. Pass a stable callback (`useCallback`), since the hook
subscribes again whenever it changes. Type the result with
`useMicdropClassification<MyResult>(...)`.
