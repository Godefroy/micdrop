...
Each VAD setting trades latency against false positives. [Silero VAD, WebRTC VAD and a volume threshold](/blog/voice-activity-detection-browser) misfire on background noise at very different rates, so the detector you pick limits how far you can tune for speed.
