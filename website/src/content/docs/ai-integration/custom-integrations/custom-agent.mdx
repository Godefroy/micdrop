---
title: 'Agent (LLM)'
description: 'The Agent class is the core abstraction for AI conversation handling in Micdrop. It provides a standardized interface for integrating various AI providers into…'
order: 58
---
...
- If hook returns a text, speaks it and adds it to the conversation as the answer, without generating
...
A message whose text is empty or made of whitespace is ignored, so a turn where the LLM only called a tool leaves no empty message behind in the conversation.
...
Emitted when a message, a tool call or a tool result is added to the
conversation.

```typescript
agent.on('Message', (message: MicdropConversationItem) => {
  console.log('New conversation item:', message)
})
```

### `ToolCall`

Emitted when a tool declared with `emitOutput` has run, with its parameters and
its output.

```typescript
agent.on('ToolCall', (toolCall: MicdropToolCall) => {
  console.log(`Tool ${toolCall.name} returned`, toolCall.output)
})
```
...
Emitted when the agent gives up generating an answer, for example after
exhausting its retries. [FallbackAgent](../fallback-strategies/agent-fallback)
listens to it to switch to the next agent.

```typescript
agent.on('Failed', () => {
  console.error('Agent failed to answer')
})
```
