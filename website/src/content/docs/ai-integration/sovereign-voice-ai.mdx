...
Gradium est le seul fournisseur souverain de synthèse vocale intégré à Micdrop, et l'article [text-to-speech français souverain avec Gradium](/blog/gradium-text-to-speech-souverain) montre comment régler la voix et ce que coûte un secours qui reste en Europe les jours de panne.
...
```bash
npm install @micdrop/server @micdrop/web @micdrop/mistral @micdrop/gladia @micdrop/gradium
```
