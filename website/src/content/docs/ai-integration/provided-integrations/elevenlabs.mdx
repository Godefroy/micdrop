---
title: 'ElevenLabs'
description: 'Add ElevenLabs streaming voices to a TypeScript voice pipeline with one npm package, and your Node server pushes the audio out as it arrives.'
order: 48
---
...
ElevenLabs and every other voice engine in Micdrop sit behind the same interface, so moving to an [ElevenLabs alternative](/blog/elevenlabs-alternatives-typescript) only means replacing `new ElevenLabsTTS(...)` with the other engine's class.
