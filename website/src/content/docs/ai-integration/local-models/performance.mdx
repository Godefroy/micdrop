...
A local call therefore answers in about 1 second with Piper or Pocket TTS, and
closer to 2 with Kokoro. When the model answers one of the automatic prompts,
the turn runs a second pass, and that pass costs more than the transcription
and the voice put together.
...
model when the session opens. The CoreML log shows why: CoreML covers 577 of
the 873 nodes and splits the graph into 88 partitions, so the run spends its
time copying tensors back and forth across those boundaries instead of
computing.
